// main.c — FasterEdge MCU PY32F002A PlatformIO / ARM GCC 入口
    ...
    while (fe_port_uart_available(0)) {
        ...
        if (c == '\n' || c == '\r') {
            if (line_overflow) {
                ...
                line_overflow = FALSE;
                *len = 0;
                return 2; // oversized line was discarded, never execute a prefix
            }
            if (line_len == 0) continue;
            if (line_len + 1 > buflen) {
                line_len = 0;
                *len = 0;
                return 2;
            }
...
int main(void) {
    ...
    for (;;) {
        char buf[96];
        u16 len;
        u8 line_status = read_line(buf, sizeof(buf), &len);
        if (line_status == 2) {
            fe_port_uart_write(0, (const u8 *)"ERR input: line too long\r\n", 26);
            continue;
        }
        if (line_status == TRUE) {
