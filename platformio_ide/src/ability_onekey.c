// ability_onekey.c — OneKeyAbility 实现（PY32F002A / Cortex-M0+ 版）
...
fe_output_t ability_onekey_dispatch(void *inst, const char *act, const char *args) {
    ...
    if (!fe_port_persistence_available())
        return fe_err(act, "persistent storage unavailable");
    ...
    if (strcmp(act, "list_tokens") == 0) {
        ...
        int w = fe_snprintf(out, sizeof(out), "{\"tokens\":[");
        if (w < 0 || (u16)w >= sizeof(out)) return fe_err(act, "response too large");
        n = (u16)w;
        for (i = 0; i < TOKEN_SLOTS; i++) {
            u32 seq; char subject[TOKEN_SUBJ_LEN];
            if (!read_token(i, &seq, subject)) continue;
            w = fe_snprintf(out + n, (u16)(sizeof(out) - n), "%s%lu", first ? "" : ",", (unsigned long)seq);
            if (w < 0 || (u16)w >= sizeof(out) - n) return fe_err(act, "response too large");
            n += (u16)w;
            first = FALSE;
        }
        w = fe_snprintf(out + n, (u16)(sizeof(out) - n), "]}");
        if (w < 0 || (u16)w >= sizeof(out) - n) return fe_err(act, "response too large");
        return fe_ok(act, out);
    }
