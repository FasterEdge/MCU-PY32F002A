...
void fe_list_data_names(const fe_atom_t *atom, char *out, u16 outlen) {
    u16 n = 0;
    u8 i;
    if (!atom || !out || outlen == 0) return;
    out[0] = 0;
    for (i = 0; i < atom->data_count && n + 1 < outlen; i++) {
        int w = fe_snprintf(out + n, (u16)(outlen - n), "%s%s", i ? "," : "", atom->data[i].name);
        if (w < 0 || (u16)w >= outlen - n) { out[outlen - 1] = 0; break; }
        n += (u16)w;
    }
}
...
void fe_list_ability_names(const fe_atom_t *atom, char *out, u16 outlen) {
    ...
    if (!atom || !out || outlen == 0) return;
    out[0] = 0;
    for (i = 0; i < atom->ability_count && n + 1 < outlen; i++) {
        int w = fe_snprintf(out + n, (u16)(outlen - n), "%s%s", i ? "," : "", atom->ability[i].name);
        if (w < 0 || (u16)w >= outlen - n) { out[outlen - 1] = 0; break; }
        n += (u16)w;
    }
}
...
fe_output_t fe_execute(fe_atom_t *atom, const char *target, const char *act, const char *args) {
    u8 isData = FALSE;
    if (!atom || !target || !act || !target[0] || !act[0])
        return fe_err(act, "invalid command");
